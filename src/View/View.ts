...
export class View {
  ...
  constructor(readonly root: Annotator, readonly svgElement: SVGSVGElement, readonly config: Config) {
    ...
    const labelText = Array.from(this.store.labelCategoryRepo.values())
      .map((it) => it.text)
      .join('');
    const connectionText = Array.from(this.store.connectionCategoryRepo.values())
      .map((it) => it.text)
      .join('');
    ...
    this.topContextLayerHeight =
      config.topContextMargin * 2 + Math.max(labelElementHeight, this.connectionFont.lineHeight);
  ...
  private get height() {
    return this.lines.reduce(
      (currentValue, line) => currentValue + line.height + this.contentFont.fontSize * (this.config.lineHeight - 1),
      20
    );
  }
  ...
  private registerEventHandlers() {
    this.textElement.onmouseup = (e) => {
      if (window.getSelection()!.type === 'Range') {
        ...
        if (this.config.contentEditable) this.contentEditor.caretChanged(e.clientY);
      }
    };
  ...
  private onContentSpliced(startIndex: number, removed: string, inserted: string) {
    if (removed !== '') this.onRemoved(startIndex, removed);
    if (inserted !== '') this.onInserted(startIndex, inserted);
  }
  ...
  private findHardLineEndsInIndex(startInLineIndex: number) {
    ...
    for (
      hardLineEndInIndex = startInLineIndex;
  ...
  private collectStyle(): SVGStyleElement {
    ...
    const textClassSelector = this.config.contentClasses.map((it) => '.' + it).join(',');
    // F*** SVG's LINE HEIGHT
    //
    // When you need it,
    // No affect it takes.
    ...
    const textStyle = `
        ...
        `;
    const labelClassSelector = this.config.labelClasses.map((it) => '.' + it).join(',');
    const labelStyle = `
        ${labelClassSelector} {
        ...
        `;
    const connectionClassSelector = this.config.connectionClasses.map((it) => '.' + it).join(',');
    const connectionLineClassSelector = this.config.connectionClasses.map((it) => '.' + it + '-line').join(',');
