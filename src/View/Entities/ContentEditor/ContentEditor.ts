...
export class ContentEditor {
  ...
  render(): [SVGPathElement, HTMLTextAreaElement] {
    ...
    this.parentSVGYOffset =
      this.view.svgElement.getBoundingClientRect().top -
      ...
      if (!this.inComposition) {
        switch (e.key) {
          ...
          case 'ArrowRight':
            ...
            break;
          case 'ArrowUp':
            if (this.line.last.isSome) --this.lineIndex;
            this.characterIndex = Math.min(this.characterIndex, this.line.content.length);
            this.avoidInLabel('forward');
            break;
          case 'ArrowDown':
            if (this.line.next.isSome) ++this.lineIndex;
            this.characterIndex = Math.min(this.characterIndex, this.line.content.length);
            this.avoidInLabel('forward');
            break;
          case 'Backspace':
          ...
          default:
            ...
            if (this.hiddenTextAreaElement.value !== '') {
              Font.Service.measureMore(
                this.view.contentFont,
                this.hiddenTextAreaElement.value,
                this.view.config.contentClasses,
                this.view.textElement
              );
  ...
  public avoidInLabel(direction: 'backward' | 'forward') {
    ...
    const labels: Array<LabelView.Entity> = Array.from(this.line.topContext.children).filter(
      (it) => it instanceof LabelView.Entity
    ) as any;
    let overlapWith = labels.find((it) => it.store.startIndex <= position - 1 && position < it.store.endIndex);
    while (overlapWith !== undefined) {
      if (direction === 'forward') ++this.characterIndex;
      else --this.characterIndex;
  ...
  caretChanged(y: number) {
    ...
    if (selectionInfo.anchorNode!.parentNode !== null) {
      ...
      if (lineY + this.view.contentFont.lineHeight <= y) {
        ...
        const lineEntity = (selectionInfo.anchorNode!.parentNode as any as { annotatorElement: Line.ValueObject })
          .annotatorElement;
        this._lineIndex = this.view.lines.indexOf(lineEntity);
